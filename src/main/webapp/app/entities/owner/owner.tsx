...
export const Owner = (props: IOwnerProps) => {
  ...
  return (
    <div>
      <h2 id="owner-heading">
        <Translate contentKey="gmsApp.owner.home.title">Owners</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="gmsApp.owner.home.createLabel">Create new Owner</Translate>
        </Link>
      </h2>
        ...
        {ownerList && ownerList.length > 0 ? (
            ...
            <tbody>
              {ownerList.map((owner, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{owner.userID}</td>
                  <td>{owner.userID ? <Link to={`gms-user/${owner.userID.id}`}>{owner.userID.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="gmsApp.owner.home.notFound">No Owners found</Translate>
            </div>
          )
        )}
      </div>
