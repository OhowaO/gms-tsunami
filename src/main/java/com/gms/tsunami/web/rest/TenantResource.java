...
/**
 * REST controller for managing {@link com.gms.tsunami.domain.Tenant}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class TenantResource {
    ...
    public List<Tenant> getAllTenants(@RequestParam(required = false) String filter) {
        if ("userid-is-null".equals(filter)) {
            log.debug("REST request to get all Tenants where userID is null");
            return StreamSupport
                .stream(tenantRepository.findAll().spliterator(), false)
                .filter(tenant -> tenant.getUserID() == null)
